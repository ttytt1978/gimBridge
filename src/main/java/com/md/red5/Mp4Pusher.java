...
import com.md.ffmpeg.videoPlayer;
...
public class Mp4Pusher implements IPusher {
    ...
    public Mp4Pusher(String mp4FileName, String rtmpPath) {
...
//            pushVideoAndAudio(mp4FileName,rtmpPath);
            pushStream(mp4FileName,rtmpPath);
//            pushDoubleStream(mp4FileName,rtmpPath);
    ...
    private  void pushVideoAndAudio(String mp4FileName, String rtmpPath) {
    ...
    private  void pushStream(String orginPath, String rtmpPath){
        // 使用rtsp的时候需要使用 FFmpegFrameGrabber，不能再用 FrameGrabber
        int width = 640, height = 480;
        FFmpegFrameGrabber frameGrabber = new FFmpegFrameGrabber(orginPath);
        Frame captured_frame = null;
        FFmpegFrameRecorder recorder = null;

        try {
            frameGrabber.start();
            recorder = new FFmpegFrameRecorder(rtmpPath, frameGrabber.getImageWidth(), frameGrabber.getImageHeight(), frameGrabber.getAudioChannels());
            ...
            while (!exit) {
                try {
                    ...
                    {
//                        System.out.println("---------------no frame..."+timer.getTimeInMillSecond() );
                        continue;
                    }
                    ...
                    timer.end();
                    System.out.println("push one frame----cost:"+timer.getTimeInMillSecond());
