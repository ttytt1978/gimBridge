...
public class DirectPusher implements IPusher {
    ...
    public DirectPusher(String originPath, String rtmpPath) {
...
//            pushDoubleStream(mp4FileName,rtmpPath);
    ...
    private  void pushVideoAndAudio(String orginPath, String rtmpPath) {
        // 使用rtsp的时候需要使用 FFmpegFrameGrabber，不能再用 FrameGrabber
        int width = 640, height = 480;
        videoPlayer player = new videoPlayer(orginPath,rtmpPath);
    ...
    private  void pushStream(String orginPath, String rtmpPath){
        ...
        //直接推送，但是修改帧时间戳为新时间戳
        long orginStartTimeStamp=-1,newStartTimeStamp=-1;
        try {
            frameGrabber.start();
...
//                    Thread.sleep(50);
                    ...
                    timer.end();
                    System.out.println("push one frame----cost:"+timer.getTimeInMillSecond()+",timestamp="+timestamp);
