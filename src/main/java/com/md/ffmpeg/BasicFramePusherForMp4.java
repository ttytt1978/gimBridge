...
//基本的推流器
public class BasicFramePusherForMp4 implements IFramePusher {
    private IFrameMerger frameMerger;
    private boolean exit=false;
    private String mp4FilePath;//推流地址
    ...
    public BasicFramePusherForMp4(String mp4FilePath)
    {
        this.mp4FilePath =mp4FilePath;
    }

    public BasicFramePusherForMp4(String mp4FilePath, IFrameMerger frameMerger)
    {
        this.mp4FilePath =mp4FilePath;
    ...
    @Override
    public void start() {
        if(null== mp4FilePath || mp4FilePath.length()<2)
    ...
    private void openRecorder()
    {
        try {
            ...
            recorder.setFormat("mp4");//只支持flv，mp4，3gp和avi四种格式，
...
//                    {
//                        System.out.println("XXXXXXXXXXX推帧器录制一帧错误时间戳...."+frame.timestamp+" ,前一帧时间戳："+lastTimeStamp);
//                        continue;
//                    }
//                    else
                        lastTimeStamp=frame.timestamp;
...
//                    {
//                        System.out.println("XXXXXXXXXXX推帧器录制一视频帧时间戳早于当前时间戳...."+frame.timestamp+" ,当前时间戳："+recorder.getTimestamp());
//                        continue;
//                    }
//                        continue;
...
//                        else
                        ...
                        if(frame.image!=null)
                            System.out.println("-----------------------推帧器成功录制一视频帧...."+frame.timestamp);
                        if(frame.samples!=null)
                            System.out.println("-----------------------推帧器成功录制一音频帧...."+frame.timestamp);
                    }catch (Exception e)
                    {
...
//                        videoFrameController.updateGrabCount();
                    ...
                    {
                        System.out.println("1秒内录制的总帧数："+pushCount);
                        pushCount=0;
