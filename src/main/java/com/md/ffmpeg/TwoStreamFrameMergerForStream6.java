...
//简单合帧器
public class TwoStreamFrameMergerForStream6 extends AbstractFrameMerger {
    ...
    public TwoStreamFrameMergerForStream6() {
        super(100);
    }

    public TwoStreamFrameMergerForStream6(int capacity) {
        super(capacity);
    }
    ...
    //选取最大的时间戳距离（但不能等于0）
    private long getLongestTimestampDistance(long d1, long d2) {
        if (d1 <= 0 && d2 > 0)
            return d2;
        if (d2 <= 0 && d1 > 0)
            return d1;
    ...
    // 从通道帧列表中剔除小于最后一帧时间戳的帧
    private  void cutChannelFramesFromList(List<ChannelFrame> channelFrames,long lastTimestamp)
    {
        for(int i=channelFrames.size()-1;i>=0;i--)
        {
            ChannelFrame frame=channelFrames.get(i);
            if(frame.frame.timestamp<lastTimestamp)
            {
                channelFrames.remove(i);
            }
        }
...
//        FFmpegFrameFilter asFilter = new FFmpegFrameFilter("aformat=sample_fmts=s16:channel_layouts=mono:sample_rates=44100,aresample=44100,asetnsamples=n=1900:p=0,anull", 1);
        ...
        while (!isStopped()) {
            ...
            //按统一时间戳顺序合并两个通道帧序列
            List<ChannelFrame> mergedFrames = mergeAndSortTwoFrames(inList1, inList2);
            //剔除小于最后一帧时间戳的帧
//            cutChannelFramesFromList(mergedFrames,unityTimestamp);
            //更新最后一帧时间戳
            if(!mergedFrames.isEmpty())
            {
                unityTimestamp=mergedFrames.get(mergedFrames.size()-1).frame.timestamp;
            }
            //更新当前段的最后一帧时间戳
            if (!inList1.isEmpty()) {
...
//                    videoFrameController.updateGrabCount();
    ...
    private synchronized void adjustMaxFrameSum() {
        synchronized (frameQue) {
            ...
            if(size<=bufferMaxCapacity*grabRatio/2)
            {
