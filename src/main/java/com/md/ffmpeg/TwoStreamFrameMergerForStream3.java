...
//简单合帧器
public class TwoStreamFrameMergerForStream3 extends AbstractFrameMerger {
    ...
    private int maxFrameSum=16;

    public TwoStreamFrameMergerForStream3() {
        super(100);
    }

    public TwoStreamFrameMergerForStream3(int capacity) {
        super(capacity);
    }
...
//            if(!((totalFrames1.size()+totalFrames2.size()>=maxFrameSum)&&(totalFrames1.size()>maxFrameSum/3)&&(totalFrames2.size()>maxFrameSum/3)))
...
//            Collections.sort(totalFrames2,comparator);
            if(isFirstFrame1&&(!totalFrames1.isEmpty()))//初始化通道1的时间戳
            {
                ...
                newLastTimestamp1=unityTimestamp;
            }
            if(isFirstFrame2&&(!totalFrames2.isEmpty()))//初始化通道2的时间戳
            {
                ...
                newLastTimestamp2 = unityTimestamp;
            }
            ...
            {
                Frame lastFrame=totalFrames2.get(totalFrames2.size()-1);
            ...
            transformTimestampForFrameList(totalFrames2,oldLastTimestamp2,newLastTimestamp2);
            ...
            //按统一时间戳顺序合并两个通道帧序列
            List<ChannelFrame> mergedFrames = mergeAndSortTwoFrames(totalFrames1, totalFrames2);
            //更新统一时间戳到最后一帧
...
//                    videoFrameController.updateGrabCount();
    ...
    private synchronized void adjustMaxFrameSum() {
        synchronized (frameQue)
        {
            ...
            int minSize=(int)(bufferMaxCapacity*ratio/2);
            int maxSize=(int)(bufferMaxCapacity*ratio*2);
            if(size<=bufferMaxCapacity*ratio/2)
            {
