...
//简单合帧器
public class TwoStreamFrameMergerForStream4 extends AbstractFrameMerger {
    ...
    private double grabRatio = 0.66;
    private double maxFrameRatio = 0.66;
    private int maxFrameSum;

    public TwoStreamFrameMergerForStream4() {
        super(100);
    }

    public TwoStreamFrameMergerForStream4(int capacity) {
        super(capacity);
    }
...
//        FFmpegFrameFilter asFilter = new FFmpegFrameFilter("aformat=sample_fmts=s16:channel_layouts=mono:sample_rates=44100,aresample=44100,asetnsamples=n=1900:p=0,anull", 1);
        ...
        while (!isStopped()) {
            ...
            //选取最小的时间戳距离（但不能等于0）
            long shortestTimestampDistance1 = 0, shortestTimestampDistance2 = 0;
            if (!totalFrames1.isEmpty()) {
                shortestTimestampDistance1 = getTimestampDistanceForFrameList(totalFrames1);
                shortestTimestampDistance1 += totalFrames1.get(0).timestamp - oldLastTimestamp1;
            }
            if (!totalFrames2.isEmpty()) {
                shortestTimestampDistance2 = getTimestampDistanceForFrameList(totalFrames2);
                shortestTimestampDistance2 += totalFrames2.get(0).timestamp - oldLastTimestamp2;
            }
            long shortestTimestampDistance = getShortestTimestampDistance(shortestTimestampDistance1, shortestTimestampDistance2);
            System.out.println("shortestTimestampDistance=" + shortestTimestampDistance);
            //按最小时间戳距离截取通道1帧列表
            List<Frame> inList1 = new ArrayList<>();
            List<Frame> outList1 = new ArrayList<>();
            for (Frame frame : totalFrames1) {
                if (frame.timestamp - oldLastTimestamp1 <= shortestTimestampDistance) {
                    inList1.add(frame);
                } else
                    outList1.add(frame);
            }
            //按最小时间戳距离截取通道2帧列表
            List<Frame> inList2 = new ArrayList<>();
            List<Frame> outList2 = new ArrayList<>();
            ...
            //变换每个帧列表的时间戳起点到统一时间戳
            transformTimestampForFrameList(inList1, oldLastTimestamp1, newLastTimestamp1);
            transformTimestampForFrameList(inList2, oldLastTimestamp2, newLastTimestamp2);
            //更新当前段的最后一帧时间戳
            if (!inList1.isEmpty()) {
                oldLastTimestamp1 = oldLastTimestamp11;
                newLastTimestamp1 = newLastTimestamp11;
            } else {
                newLastTimestamp1 += shortestTimestampDistance;//空通道则滑动一段时间戳
            }
            if (!inList2.isEmpty()) {
                oldLastTimestamp2 = oldLastTimestamp22;
                newLastTimestamp2 = newLastTimestamp22;
            } else {
                newLastTimestamp2 += shortestTimestampDistance;//空通道则滑动一段时间戳
            }
            printFrameList(totalFrames1, "totalFrames1");
            printFrameList(inList1, "inList1");
            printFrameList(outList1, "outList1");
            printFrameList(totalFrames2, "totalFrames2");
            printFrameList(inList2, "inList2");
            printFrameList(outList2, "outList2");

            //对本次混合音频效果进行设置
            audioMerger.setNeedMerged(haveAudioFrameInFrameList(inList1) && haveAudioFrameInFrameList(inList2));
            int videoChannelNumber=getVideoFrameChannelNumber(inList1,inList2);
            List<ChannelFrame> mergedFrames = mergeAndSortTwoFrames(inList1, inList2);//按统一时间戳顺序合并两个通道帧序列
            totalFrames1 = outList1;
            totalFrames2 = outList2;
            //插入视频帧
//            if(!mergedFrames.isEmpty())
//            {
//                long s=mergedFrames.get(0).frame.timestamp;
//                long t=mergedFrames.get(mergedFrames.size()-1).frame.timestamp;
//                List<ChannelFrame> videoFrames=splitVideoChannelFramesFromList(mergedFrames);
//                if(!videoFrames.isEmpty())
//                {
//                    updateTimestampForChannelFramesList(videoFrames,s,t);
//                    mergedFrames.addAll(videoFrames);
//                    Comparator<ChannelFrame> comparator1 = new Comparator<ChannelFrame>() {
//                        @Override
...
//                        }
//                    };
//                    Collections.sort(mergedFrames,comparator1);
//                }
//            }
...
//                    videoFrameController.updateGrabCount();
                ...
                if (channelFrame.channel ==videoChannelNumber && channelFrame.frame.image != null)//合成新的视频帧
...
//            }
    ...
    //统计给定一组帧列表中视频帧数量
    private int getVideoFrameSumInFrameList(List<Frame> frameList) {
        int sum=0;
        for (Frame frame : frameList)
            if (frame != null && frame.image != null)
               sum++;
    ...
    //选择一个通道作为视频帧输出通道
    private int getVideoFrameChannelNumber(List<Frame> channel1FrameList,List<Frame> channel2FrameList)
    {
        int sum1=getVideoFrameSumInFrameList(channel1FrameList);
//        int sum2=getVideoFrameSumInFrameList(channel2FrameList);
        if(sum1>0)
            return 1;
        return 2;
    }
...
//        }
    ...
    private class TwoVideoFrameMerger {
        ...
        private double scaleRation = 0.38;
        private double marginRation=0.015;
...
//           return (BufferedImage) bufImg.getScaledInstance(width,height,Image.SCALE_FAST);
        ...
        private BufferedImage combineTwoImagesToRightTop(BufferedImage smallImage, BufferedImage bigImage) {
            ...
            graphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER));
            //叠加图像
            int marginx = (int) (bigImage.getWidth() * marginRation);
            int marginy = (int) (bigImage.getHeight() * marginRation);
            ...
            graphics.setStroke(new BasicStroke((int) (bigImage.getWidth() * marginRation)));
