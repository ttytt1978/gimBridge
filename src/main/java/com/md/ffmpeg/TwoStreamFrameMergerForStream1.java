...
//简单合帧器
public class TwoStreamFrameMergerForStream1 extends AbstractFrameMerger {
    ...
    public TwoStreamFrameMergerForStream1() {
        super(100);
    }

    public TwoStreamFrameMergerForStream1(int capacity) {
        super(capacity);
    }
    ...
    //变换每个帧列表的时间戳起点到统一时间戳
    private void transformTimestampForFrameList(List<Frame> frames,long startTimestamp)
    {
      if(frames.isEmpty())
          return;
      long oldStartTimestamp=frames.get(0).timestamp;
      for(Frame frame:frames)
      {
          frame.timestamp=frame.timestamp-oldStartTimestamp+startTimestamp;
      }
    }
...
//        FFmpegFrameFilter asFilter = new FFmpegFrameFilter("aformat=sample_fmts=s16:channel_layouts=mono:sample_rates=44100,aresample=44100,asetnsamples=n=1900:p=0,anull", 1);
        ...
        while (!isStopped()) {
            ...
            //选取最小的时间戳距离（但不能等于0）
            long shortestTimestampDistance=getShortestTimestampDistance(totalFrames1,totalFrames2);
            //变换每个帧列表的时间戳起点到统一时间戳
            transformTimestampForFrameList(totalFrames1,unityTimestamp);
            transformTimestampForFrameList(totalFrames2,unityTimestamp);
            //截取每个帧列表的一段，使其不超过最小时间戳距离
            totalFrames1=cutFrameListInShortestTimestampDistance(totalFrames1,shortestTimestampDistance);
            totalFrames2=cutFrameListInShortestTimestampDistance(totalFrames2,shortestTimestampDistance);
            //更新统一时间戳
...
//                }
                ...
                if (channelFrame.channel == 1 && channelFrame.frame.samples != null)//通道2中的音频帧
                {
                    addMergedFrameToQue(channelFrame.frame);
                    continue;
                }

                if (channelFrame.channel == 1 && channelFrame.frame.image != null)//通道2中的视频帧
                {
                    addMergedFrameToQue(channelFrame.frame);
                }
...
//        }
    ...
    private List<ChannelFrame> mergeAndSortTwoFrames(List<Frame> frames1, List<Frame> frames2) {
        ...
        for (ChannelFrame frame : resultList) {
            System.out.print("(" + frame.channel + "," + frame.frame.timestamp + ")  ");
        }
...
//        private String filterStr = "[0:a]aformat=sample_fmts=s16:channel_layouts=FL:sample_rates=48000[audio1];[1:a]aformat=sample_fmts=s16:channel_layouts=FR:sample_rates=48000[audio2];[audio1][audio2]amix=inputs=2[mix1];[mix1]atempo=1.08[a]";
    ...
    private class TwoVideoFrameMerger {
        ...
        private double scaleRation = 0.58;
