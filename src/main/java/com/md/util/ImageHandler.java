...
public class ImageHandler {
	...
	public ImageHandler(byte[] imageData) {
		try {
			bufferedImage = bytesToImage(imageData);
			fileName = "c:\\temp.png";
		} catch (Exception e) {
			e.printStackTrace();
			bufferedImage = null;
	...
	// 将一个字节数组保存成一个图像对象
	private BufferedImage bytesToImage(byte[] data) throws RuntimeException {
		try {
	...
	// 不失真地将图像缩放到刚好包裹给定的宽度和高度
	public void resizeToFit(int minWidth, int minHeight) throws RuntimeException {
		try {
			if (minHeight <= 1 || minHeight <= 1)
				throw new RuntimeException("宽度或高度参数错误！");
			int width = bufferedImage.getWidth();
			int height = bufferedImage.getHeight();
			double max1 = minWidth * 1.0 / width;
			double max2 = minHeight * 1.0 / height;
			double scale = max1 > max2 ? max1 : max2;
			...
			throw new RuntimeException("缩放图像包裹矩形失败：" + e.getMessage());
		}
	...
	public void combineWithPicture(ImageHandler image2) {
		combineWithPicture(image2.getAsBufferedImage(), 2, null);
	}

	public void combineWithPicture(ImageHandler image2, int jump) {
		combineWithPicture(image2.getAsBufferedImage(), jump, null);
	}

	public void combineWithPicture(ImageHandler image2, Color ignoreColor) {
		combineWithPicture(image2.getAsBufferedImage(), 2, ignoreColor);
	}

	public void combineWithPicture(ImageHandler image2, int jump, Color ignoreColor) {
	...
	{
		try {			
			int bigWidth = bufferedImage.getWidth();
			int bigHeight = bufferedImage.getHeight();			
			//先生成一个大的图像
			BufferedImage result = new BufferedImage(bigWidth+borderWidth*2,bigHeight+borderWidth*2,BufferedImage.TYPE_INT_ARGB);
			ImageHandler bigHandler=new ImageHandler(result);
			bigHandler.overlay(this, borderWidth, borderWidth);//叠加原来的内容
			bufferedImage=bigHandler.bufferedImage;//替换当前内容
			//绘制边框线
			bigWidth = bufferedImage.getWidth();
			bigHeight = bufferedImage.getHeight();
			//上边线
			for(int x=0;x<bigWidth;x++)
				for(int y=0;y<borderWidth;y++)
					bufferedImage.setRGB(x, y, borderColor);			
			//下边线
			for(int x=0;x<bigWidth;x++)
				for(int y=0;y<borderWidth;y++)
					bufferedImage.setRGB(x, bigHeight-y-1, borderColor);	
			//左边线
			...
			for(int y=0;y<bigHeight;y++)
				for(int x=0;x<borderWidth;x++)
					bufferedImage.setRGB(bigWidth-x-1, y, borderColor);	

		} catch (Exception e) {
			throw new RuntimeException("为图像四周设置给定颜色的边框线失败：" + e.getMessage());
		}
	}

	// 叠加一个小图像
	public void overlay(ImageHandler smallImage, int left, int top) throws RuntimeException {
		try {
			int bigWidth = bufferedImage.getWidth();
			int bigHeight = bufferedImage.getHeight();

			int smallWidth = smallImage.bufferedImage.getWidth();
			int smallHeight = smallImage.bufferedImage.getHeight();

			int overlayWidth = smallWidth > (bigWidth - left) ? (bigWidth - left) : smallWidth;
			int overlayHeight = smallHeight > (bigHeight - top) ? (bigHeight - top) : smallHeight;
			int endx = left + overlayWidth;
			int endy = top + overlayHeight;
			for (int y = top; y < endy; y++) {
				for (int x = left; x < endx; x++) {
					int rgb = smallImage.bufferedImage.getRGB(x - left, y - top);
					bufferedImage.setRGB(x, y, rgb);
				}
			}

		} catch (Exception e) {
			throw new RuntimeException("叠加图像失败：" + e.getMessage());
		}
	...
	public static void main(String[] args) {
		...
		ImageHandler image = new ImageHandler("c:/pics/p1.jpg");
