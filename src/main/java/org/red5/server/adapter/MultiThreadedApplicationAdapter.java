 ...
 * 
 * applicationadapter类用作Red5应用程序的基类。
 * 它提供了使用SharedObjects和流的方法，以及连接和调度服务。
 * <BR>applicationadapter是应用程序级别的iscope。
 * 要处理应用程序中的流式处理，您应该实现@link istreamawarescopehander接口和实现处理方法。<BR>
 * 应用程序适配器为您提供了有用的事件处理程序，可用于截取流、授权用户等。此外，所有
 * 子类中添加的方法可以使用netconnection.call方法从客户端调用。与闪存媒体服务器不同，闪存媒体服务器需要
 * 将方法保留在服务器端的客户机对象上，red5提供了更方便的方法来添加方法
...
public class MultiThreadedApplicationAdapter extends StatefulScopeWrappingAdapter implements ISharedObjectService, IBroadcastStreamService, IOnDemandStreamService, ISubscriberStreamService, ISchedulingService, IStreamSecurityService, ISharedObjectSecurityService, IStreamAwareScopeHandler, ApplicationMXBean {
    ...
    /**
     * Register a com.listener that will get notified about application events.
     * 
     * @param listener
     *            object to register
     */
