...
public class ServerStream extends AbstractStream implements IServerStream, IFilter, IPushableConsumer, IPipeConnectionListener {
    ...
    public void saveAs(String name, boolean isAppend) throws IOException {
        // one recording com.listener at a time via this entry point
        if (recordingListener == null) {
            IScope scope = getScope();
            // create a recording com.listener
            ...
            if (listener.init(scope, name, isAppend)) {
                ...
                // set as primary com.listener
                recordingListener = new WeakReference<>(listener);
                // add as a com.listener
                addStreamListener(listener);
                // start the com.listener thread
                listener.start();
            } else {
                log.warn("Recording com.listener failed to initialize for stream: {}", name);
            }
        } else {
            log.info("Recording com.listener already exists for stream: {}", name);
        }
    }
