 ...
 * 在 Mina中 Transport 为 RTMP设置 监听端口和 线程池
 * <br>
...
public class RTMPMinaTransport implements RTMPMinaTransportMXBean {
    ...
    public void start() throws Exception {
        initIOHandler();
        //1、设置缓存
        IoBuffer.setUseDirectBuffer(!useHeapBuffers); // this is global, oh well 分配缓存
        if (useHeapBuffers) {
        ...
        if (enableDefaultAcceptor) {
            ...
            //构建默认监听，同时设置执行线程数量   4核电脑  ioThreads=8
        ...
        try {
        	// 绑定全部地址到端口，如 localhost,172.0.0.1,*
    ...
    /**
     * Returns the current statistics as a com.json formatted string.
     * 
     * @return com.json
     */
